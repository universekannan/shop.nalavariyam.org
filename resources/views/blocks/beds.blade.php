    ...
    <div class="card card-primary card-outline card-outline-tabs">
              <div class="card-header p-0 border-bottom-0">
                <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
					...
					<div class="col-sm-5">
                   <center> <div class="nav-link">Bde List</div></center>
					</div>
					<div class="col-sm-4">
					...
					<div class="col-sm-1">
						<td>
<button type="button" class="btn btn-block btn-secondary" data-toggle="modal" data-target="#addroom"><i class="fa fa-plus"></i> Add</button>
</td>
					</div>
                </ul>
              </div>
...
<tbody>
 @foreach($managebeds as $key=>$managebedslist)
                      <tr id="arrayorder_<?php echo $manageroomlist->id?>">
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $managebedslist->room_name }}</td>
                        <td>B {{ $managebedslist->room_name }}</td>
<td><?php echo $block_id = $manageroomlist->block_id; ?></td>
                        @if($managebedslist->status == 1)
                            <td>Active</td>
                        @else
                            <td>Inactive</td>
                        @endif     
						 <td>
                           <button type="button" data-toggle="modal" data-target="#edit{{ $managebedslist->id }}"  class="col-md-6 btn btn-block btn-info"><i class="fa fa-edit"></i></button>
                        </td>
						 <td>
						 <a href="bed/{{ $manageroomlist->id }}" type="button"class="col-md-5 btn btn-block btn-info"><i class="fa fa-eye"> Bed</i></a>
						...
						<div class="modal fade" id="addroom">
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <input type="hidden" class="form-control" name="block_id" value="{{ $block_id }}"/>
